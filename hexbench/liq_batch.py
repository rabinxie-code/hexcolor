...
def quantize_many_observed(
    samples: list[np.ndarray], speed: int, workers: int, min_cluster_ratio: float
) -> list[tuple[np.ndarray, np.ndarray]] | None:
    """Quantize boxes and map them to final observed colors in one native call."""
    ...
    packed = np.ascontiguousarray(np.concatenate(samples, axis=0), dtype=np.uint8)
    palettes = np.zeros((len(samples), 5, 3), dtype=np.uint8)
    weights = np.zeros((len(samples), 5), dtype=np.float64)
    sizes = np.zeros(len(samples), dtype=np.uint8)
    library_path = Path(imagequant._libimagequant.__file__).as_posix().encode()
    status = lib.liq_quantize_many_observed(
        library_path,
        ffi.from_buffer("uint8_t[]", packed),
        ffi.from_buffer("size_t[]", offsets),
        len(samples),
    ...
    if status != 0:
        raise RuntimeError(f"native libimagequant observed batch failed with code {status}")
    return [
        (palettes[index, : sizes[index]], weights[index, : sizes[index]])
        for index in range(len(samples))
    ]
